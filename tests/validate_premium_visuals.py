...
for slug, visual in (
    ...
    if not hero_image.startswith("/assets/images/premium/") or not (ROOT / hero_image.lstrip("/")).is_file():
...
for slug, visual in (
    ...
    if '"heroImage":"/assets/images/premium/' not in demo:
        errors.append(f"{slug} must use a verified photographic hero image")
...
if not aerolustre_path.is_file():
    ...
    if '"heroImage":"/assets/images/premium/' not in aerolustre:
